...
@Component({
  selector: 'app-add-match-video',
  templateUrl: './add-match-video.component.html',
  styleUrls: ['../leagues.scss']
})
export class AddMatchVideoComponent implements OnInit, OnDestroy {
  ...
  onInsertInit()
  {
    this.addMatchVideo = this.fb.group({
      title: ['', Validators.required],
      sub_title: [this.subtitle, Validators.required],
      time: ['90 mins',Validators.required],
  ...
  onTypeChange(event: any): void {
    ...
    const selectedType = this.types.find((type) => type.id == this.selectedTypeId);

    if (selectedType) {
      this.addMatchVideo.get('title')?.setValue(selectedType.name); // Update title with the selected type name
    }
    this.loadMatchDetails(this.selectedTypeId)
  }
  onInsert(objValue:any){
    ...
    // return false;
     this.services.addMatchVideo(objValue)
       .pipe(first())
       .subscribe(
         data => {
          //  console.log(data);
   ...
   ngOnInit(): void {
    ...
    this.loadMatchDetails(0)
    this.getStates();  
    // Listen to state_id value changes to populate cities
    // this.addMatchVideo.get('state_id')?.valueChanges.subscribe(stateId => {
    //   if (stateId) {
...
uploadedThumbnail = ''
  loadMatchDetails(typeId: number) {
    this.http.get(`${environment.apiUrl}/admin/match/edit/${this.id}`).subscribe((data: any) => {
      this.uploadedThumbnail = typeId == 2 ? data.data.match?.goal_banner : typeId == 1 ? data.data.match?.highlight_banner : ''
      this.teamA = data.data.team[0]?.Club?.name    
      this.teamB = data.data.team[1]?.Club?.name    
      this.subtitle = this.teamA + ' vs ' + this.teamB
      this.square_image_path = this.uploadedThumbnail;
      console.log(this.subtitle)
        this.cdr.detectChanges();
        this.addMatchVideo.patchValue({
