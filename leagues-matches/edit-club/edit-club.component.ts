...
export class EditClubComponent implements OnInit, OnDestroy {
  ...
  onInsert(objValue:any){
    ...
    if(this.square_image_path){
     ...
     this.services.editClub(objValue)
       .pipe(first())
       .subscribe(
         data => {
           console.log(data);
  ...
  loadClubDetails() {
    this.http.get(`${environment.apiUrl}/admin/club/edit/${this.clubId}`).subscribe((data: any) => {
      ...
      if (this.formData) {
        ...
        if (this.formData.state_id && this.formData.state_id > 0) {
          console.log('State ID:', this.formData.state_id);
      
          this.getCities(this.formData.state_id)
        }
          // .subscribe({
          //   next: (cities: any) => {
          //     console.log('Fetched cities:', cities);
              
              // Update city_id if it exists in formData
              // if (this.formData.city_id) {
              //   this.editClub.patchValue({ city_id: this.formData.city_id });
              // }
        ...
        //   });
        // } else {
        //   console.warn('Invalid or missing state_id:', this.formData.state_id);
        // }
 ...
 this.editClub.get('state_id')?.valueChanges.subscribe(stateId => {
      if (stateId) {
