...
export class AddVideoCategoryComponent implements OnInit, OnDestroy {
  ...
  onInsertInit()
  {
    this.addVideoCategory = this.fb.group({
  ...
  onInsert(objValue:any){
    ...
    // return false;
     this.services.addVideoCategory(objValue)
       .pipe(first())
       .subscribe(
         data => {
          //  console.log(data);
           if (data && data.success==true) {
             ...
             this.addVideoCategory.reset();
             this.square_image_path = ""
             this.onRemoveSquare(this.squareFile)
             this.toastr.success('Video Category Added successfully', 'Add Video Category',{
               timeOut: 3000,
               progressBar:true,
               tapToDismiss:true,
           ...
           // alert(data.message);
            this.isLoading$.next(false);
             this.toastr.error("Video Category creation process failed.", 'Add Video Category',{
               timeOut: 3000,
               progressBar:true,
               tapToDismiss:true,
               toastClass: 'flat-toast ngx-toastr'
         ...
         error => {
           this.toastr.error("Internal server error, Please try again after sometime.", 'Add Video Category',{
             timeOut: 3000,
             progressBar:true,
             tapToDismiss:true,
             toastClass: 'flat-toast ngx-toastr'
  ...
  onPaidChange(value: string) {
    ...
    if (!this.isPaid) {
      this.addVideoCategory.patchValue({ subscription_plan_id: '' }); // Reset dropdown if "No" is selected
    }
  }
  ...
  loadMasterClassPlans() {
   ...
   ngOnInit(): void {
    ...
    // this.addVideoCategory.get('state_id')?.valueChanges.subscribe(stateId => {
    //   if (stateId) {
  ...
  onSelectSquare(event:any) {
    ...
    if(this.squareFile)
      this.checkBeforeUpload(this.squareFile,1,1200,675,1200,675,1200/675,
