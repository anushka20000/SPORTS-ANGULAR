...
highlightBannerFile: File | null = null;
  ...
  constructor(private cdr: ChangeDetectorRef, private services:MatchService, private uploadService:UploadService,
       ...
       .subscribe(
         data => {
           ...
           if (data && data.success==true) {
             ...
             this.toastr.success('Match Updated successfully', 'Edit Match',{
               timeOut: 3000,
               progressBar:true,
               tapToDismiss:true,
           ...
           // alert(data.message);
            this.isLoading$.next(false);
             this.toastr.error("Match update process failed.", 'Edit Match',{
               timeOut: 3000,
               progressBar:true,
               tapToDismiss:true,
               toastClass: 'flat-toast ngx-toastr'
...
return formattedDate
  ...
  loadMatchDetails() {
    this.http.get(`${environment.apiUrl}/admin/match/edit/${this.matchId}`).subscribe((data: any) => {
      ...
      if (this.formData) {
        ...
        this.editMatch.patchValue({
          league_id: this.formData.match?.league_id,
          group_id: this.formData.match?.group_id,
          teamA_id: this.formData.team[0].club_id,
          teamB_id: this.formData.team[1].club_id,
          paid: this.formData.match.paid,
          date: this.getFormattedDate(Number(this.formData.match.date)),
          channel_id: this.formData.match?.ingest_link,
          sports_type: this.formData.match?.sports_type,
            location:this.formData.match?.location,
            ground_name:this.formData.match?.ground_name,
            preview_seconds:this.formData.match?.preview_seconds,
   ...
   ngOnInit(): void {
    ...
    // if(this.matchId) {
      ...
      // if(this.clubId) {
        ...
        if(this.matchId){
      console.log(this.matchId, 'hello')
      this.loadMatchDetails()
    }
    ...
    // this.editMatch.get('state_id')?.valueChanges.subscribe(stateId => {
    //   if (stateId) {
...
case 4: {
  ...
  case 5: {
    ...
    if(this.squareFile5)
    this.checkBeforeUpload(this.squareFile5,type,292,165,292,165,292/165,
