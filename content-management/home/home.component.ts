...
onEditFormClose() {
  ...
  private loadGrid() {
    let url = `${environment.apiUrl}/admin/home`;
    ...
    this.dtOptions = {
      ...
      columns: [
      ...
      
        {
          ...
          orderable:false,
          render: function (data: any, type: any, full: any) {
            return ((full.typeid == 1 || full.typeid == 2 || full.typeid == 3 || full.typeid == 4 )?'<button  title="Items" action="item_grid" data-type="'+ full.typeid +'" data-id="'+full.id+'"  class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">' +
              '<span class="svg-icon svg-icon-3">' +
              '<img width="16" height="16" action="item_grid" data-type="'+ full.typeid +'" data-id="'+full.id+'" ' +
              'src="./assets/media/icons/tutopia/visible.svg"></span></button>': '');
          }
        }],
    };
  }
  ...
  ngAfterViewInit(): void {
    ...
    this.renderer.listen('document', 'click', (event) => {
      ...
      // }
      if (element.hasAttribute("data-id") && element.hasAttribute("data-type") && element.hasAttribute("action") ) {
        console.log(element.getAttribute("data-type"),element.getAttribute("action"))
        this.onAction(element.getAttribute("data-id"),element.getAttribute("action"), element.getAttribute("data-type"));
      }
    });

  }
  type = 0
  private onAction(id:string, action:string, type: any):void {
    ...
    switch(action) {
      ...
      case "item_grid": {
        this.router.navigate([`cms/home/items/${this.selectedClubId}/${this.type}`]);
