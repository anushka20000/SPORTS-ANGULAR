...
@Component({
  selector: 'app-add-subscription-request',
  templateUrl: './add-subscription-request.component.html',
})
export class AddSubscriptionRequestComponent implements OnInit, OnDestroy {
      ...
      .subscribe(
        data => {
          ...
          if (data && data.success==true) {
            this.addForm.reset();
            this.toastr.success('Subscription request added successfully', 'Add Subscription request',{
              timeOut: 3000,
              progressBar:true,
              tapToDismiss:true,
          ...
          {
            this.toastr.error("Subscription plan could not be saved, error:"+data.message,
        ...
        error => {
          this.toastr.error("Internal server error, Please try again after sometime.",
